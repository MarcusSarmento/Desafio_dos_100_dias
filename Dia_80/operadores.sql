...
-- Recuperar o cpf do supervisor, o número de funcionários desse supervisor e o salário médio para os supervisores que tem 3 ou mais funcionários
select cpf_supervisor, count(*), avg(salario) from funcionario group by cpf_supervisor having count(*)>=3;

-- Recuperar o nome de todos os funcionários que tem dois ou mais dependentes
select pnome, unome from funcionario, dependente where cpf = fcpf group by unome, pnome having count(*)>=2;
